...
- **Global scope**: `/root/.openclaw/workspace/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/root/.openclaw/workspace/.claude/notes/`)
...
2. Glob `/root/.openclaw/workspace/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/root/.openclaw/workspace/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/root/.openclaw/workspace/.claude/notes/` — works in any directory
