...
<template>
@/root/.openclaw/workspace/.claude/get-shit-done/templates/UAT.md
</template>
...
```bash
INIT=$(node "/root/.openclaw/workspace/.claude/get-shit-done/bin/gsd-tools.cjs" init verify-work "${PHASE_ARG}")
...
```bash
CHECKPOINT=$(node "/root/.openclaw/workspace/.claude/get-shit-done/bin/gsd-tools.cjs" uat render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
node "/root/.openclaw/workspace/.claude/get-shit-done/bin/gsd-tools.cjs" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
- Follow @/root/.openclaw/workspace/.claude/get-shit-done/workflows/diagnose-issues.md
