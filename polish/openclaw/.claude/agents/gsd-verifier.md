...
node "/root/.openclaw/workspace/polish/openclaw/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(node "/root/.openclaw/workspace/polish/openclaw/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "$PHASE_NUM" --raw)
```
...
```bash
ARTIFACT_RESULT=$(node "/root/.openclaw/workspace/polish/openclaw/.claude/get-shit-done/bin/gsd-tools.cjs" verify artifacts "$PLAN_PATH")
```
...
```bash
LINKS_RESULT=$(node "/root/.openclaw/workspace/polish/openclaw/.claude/get-shit-done/bin/gsd-tools.cjs" verify key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(node "/root/.openclaw/workspace/polish/openclaw/.claude/get-shit-done/bin/gsd-tools.cjs" summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(node "/root/.openclaw/workspace/polish/openclaw/.claude/get-shit-done/bin/gsd-tools.cjs" verify commits $COMMIT_HASHES)
fi
