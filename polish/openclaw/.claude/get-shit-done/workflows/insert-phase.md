...
```bash
INIT=$(node "/root/.openclaw/workspace/polish/openclaw/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "/root/.openclaw/workspace/polish/openclaw/.claude/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
